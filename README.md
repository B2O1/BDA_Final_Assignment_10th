...
| 팀원    | 소감                                                                                                                                                                                                                       |
|--------|---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| 김혜원 | 본 분석을 통해 정제되지 않은 데이터를 도메인 맥락에 맞게 전처리하는 과정이 데이터 분석 전반의 신뢰도를 좌우한다는 점을 깊이 체감했다. 잘못된 값의 통일, 결측치 처리, 비정합 그룹 제거 등 단순해 보이는 정제 과정 역시 EDA를 통해 확인하지 않았으면 놓치기 쉽다는 것을 깨달았으며, EDA와 전처리가 잘못될 경우 이후 모든 분석 결과가 왜곡될 수 있음을 경험했다. 정제된 데이터를 바탕으로 학회원의 개인 특성을 프로파일링하고, 조별활동 참여 여부와 이탈 양상을 시각화뿐만 아니라 통계 분석과 회귀 분석까지 확장해 확인할 수 있었다. 이를 통해 보다 입체적으로 이해할 수 있었으며, 단순 현황 분석을 넘어 변수 간 관계를 검증하는 분석의 중요성을 느꼈다. 또한 점수 추세 분석을 위해 기수별 조별활동 점수 기준표를 직접 비교하였다. 이 과정은 분석 기준의 일관성이 얼마나 중요한지를 깨닫게 해주었으며, 분석 목적에 맞는 데이터 정의를 끝까지 고민하고 설명해냈다는 점에서 큰 성취감을 느낄 수 있었다. |
